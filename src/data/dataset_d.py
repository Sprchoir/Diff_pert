...
class DecoderDataset(Dataset):
    ...
    - Y_full:  full-dimension data, used as ground truth 
    """
    def __init__(self, configs, Y_low_gen, Y_full, embeddings, Y_low_real=None, split=None):
        super().__init__()
        # Load data first
    ...
    def __getitem__(self, idx):
        return self.Y_low[idx,:], self.Y_full[idx,:], idx, self.embeddings[idx,:]
    
    def pair(self):
        # Pair the generated low-dim data with real full-dim data and embedding data
        ...
        self.embeddings = torch.cat([self.embeddings, self.embeddings.repeat((self.n_gen, 1))], dim=0)
    ...
    def _split(self, split):
        ...
        if split == "train":
            self.Y_low, self.Y_full, self.embeddings = self.Y_low[:n_train], self.Y_full[:n_train], self.embeddings[:n_train]
        elif split == "val":
            self.Y_low, self.Y_full, self.embeddings = self.Y_low[n_train:n_train+n_val], self.Y_full[n_train:n_train+n_val], self.embeddings[n_train:n_train+n_val]
